  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='flex gap-3'>
        <div onClick={()=>setSizes(prev => prev.includes("S")? prev.filter(item=> item!== "S"): [...prev,"S"] )} className={` border ${sizes.includes("S")? "bg-pink-100":"bg-slate-200"}`}>
          <p className=' px-3 py-1 cursor-pointer'>S</p>
        </div>
        <div onClick={()=>setSizes(prev => prev.includes("M")? prev.filter(item=> item!== "M"): [...prev,"M"] )} className={` border ${sizes.includes("M")? "bg-pink-100":"bg-slate-200"}`}>
          <p className=' px-3 py-1 cursor-pointer'>M</p>
        </div>
        <div onClick={()=>setSizes(prev => prev.includes("L")? prev.filter(item=> item!== "L"): [...prev,"L"] )} className={` border ${sizes.includes("L")?  "bg-pink-100":"bg-slate-200"}`}>
          <p className=' px-3 py-1 cursor-pointer'>L</p>
        </div>
        <div onClick={()=>setSizes(prev => prev.includes("XL")? prev.filter(item=> item!== "XL"): [...prev,"XL"] )} className={` border ${sizes.includes("XL")?  "bg-pink-100":"bg-slate-200"}`}>
          <p className='px-3 py-1 cursor-pointer'>XL</p>
        </div>
        <div onClick={()=>setSizes(prev => prev.includes("XXL")? prev.filter(item=> item!== "XXL"): [...prev,"XXL"] )} className={` border ${sizes.includes("XXL")? "bg-pink-100":"bg-slate-200"}`}>
    ...
    <button disabled={loading} className='cursor-pointer w-28 py-3 mt-4 bg-black text-white' type="submit">ADD</button>
    </form>
  )
}
